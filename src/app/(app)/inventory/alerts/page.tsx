...
// Removed Tabs, TabsList, TabsTrigger, usePathname as navigation is now in sidebar
...
export default function LowStockAlertsPage() {
  ...
  useEffect(() => {
    const filtered = allStockItems.filter(item => item.quantity <= item.reorderLevel && item.quantity > 0); 
  ...
  return (
    <div className="space-y-6">
       <div className="flex items-center justify-between">
         ...
         <Button size="sm" variant="outline" asChild className="text-muted-foreground hover:text-foreground border-input hover:bg-accent">
       ...
       <Card className="shadow-sm border border-border bg-card text-card-foreground">
         <CardHeader className="border-b border-border">
           <CardTitle className="flex items-center gap-2 text-foreground"><BellRing className="h-5 w-5 text-orange-500 dark:text-orange-400"/> Items Below Reorder Level</CardTitle>
         ...
         <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="overflow-x-auto">
               <Table>
                <TableCaption className="py-4 text-muted-foreground">Inventory items at or below their set reorder level.</TableCaption>
                <TableHeader>
                ...
                {lowStockAlertItems.length > 0 ? (
                    ...
                    <TableRow key={item.id} className="bg-orange-50 dark:bg-orange-900/20 hover:bg-orange-100 dark:hover:bg-orange-800/30 border-orange-200 dark:border-orange-700/50">
                        ...
                        <TableCell className="font-bold text-orange-600 dark:text-orange-400 py-2 px-3">{item.quantity}</TableCell>
                        <TableCell className="text-foreground py-2 px-3">{item.reorderLevel}</TableCell>
